...
import { type DriverMatchItem, listMyDriverMatches } from "@/features/matching/api";
...
export function DriverMyMatchesPage() {
  ...
  const loadMyMatches = async (mode: "initial" | "refresh" = "initial") => {
    if (mode === "refresh") {
  ...
  return (
    <PageScaffold title="운행" backgroundColor={backgroundColor} scroll={false}>
      {isLoading ? (
        <AppSpinner label="내 매칭 목록을 불러오는 중입니다." />
      ) : errorMessage ? (
        <AppErrorState
          title="내 매칭 목록을 불러오지 못했어요"
        ...
        <FlatList
          ...
          ListEmptyComponent={<AppEmptyState title="내 매칭이 없습니다." description="수락된 매칭이 있으면 표시됩니다." />}
          refreshControl={
            <RefreshControl
