                  ...
                  <div key={key} className="flex items-center justify-between text-sm border-b border-border pb-2 last:border-b-0">
                    <span className="text-muted-foreground capitalize">
                      {key === "plateNumber" ? "차량번호" :
                       ...
                       key === "manufacturingYear" ? "제조년도" :
                       key === "owner" ? "소유자" :
                       key === "registerDate" ? "등록일" :
                       key === "insuranceCompany" ? "보험사" :
                       key === "policyNo" ? "증권번호" :
                       key === "effectiveDate" ? "유효기간(시작)" :
                       key === "expiryDate" ? "유효기간(종료)" :
                       key === "coverageAmount" ? "보험금액" :
                       key === "inspectionDate" ? "검사일" :
                       key === "nextInspection" ? "다음검사일" :
                       key === "result" ? "검사결과" :
                       key === "capacity" ? "적재용량" :
                       key}
                    </span>
