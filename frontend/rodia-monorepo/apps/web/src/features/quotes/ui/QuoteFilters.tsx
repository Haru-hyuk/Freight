  ...
  return (
    <Card className="rounded-lg border border-border bg-background">
      <CardHeader className="space-y-1">
        <CardTitle className="text-base font-semibold text-foreground">견적 필터</CardTitle>
        <p className="text-sm text-foreground">견적 상태와 합짐 허용 여부 기준으로 조회합니다.</p>
      </CardHeader>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div className="space-y-2">
            <Label>합짐 허용</Label>
            <Tabs
              value={value.allowCombine}
              onValueChange={(allowCombine) => onChange({ ...value, allowCombine: allowCombine as QuoteFilterValue["allowCombine"] })}
            >
              <TabsList className="w-full border border-border bg-muted">
                <TabsTrigger value="all" className="flex-1">전체</TabsTrigger>
                <TabsTrigger value="yes" className="flex-1">허용</TabsTrigger>
                <TabsTrigger value="no" className="flex-1">비허용</TabsTrigger>
              </TabsList>
            </Tabs>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-[1fr_auto]">
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="견적 ID, 화주, 출발지, 도착지, 화물종류"
            />
          </div>

          <div className="flex items-end justify-end gap-2">
            <Button type="button" variant="secondary" onClick={onSubmit} disabled={loading}>조회</Button>
            <Button
              type="button"
